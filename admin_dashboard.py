...
class AdminDashboardPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # ✅ IMPORTANT: Create UI containers first
        self.create_sidebar()
        self.create_content()

        # ✅ Now safe to show default page
        self.set_active_button("🏠   Dashboard")
        self.show_dashboard_page()

    # -------------------- ROUTE LIFECYCLE --------------------
    def on_show(self):
        if not self.controller.session.get("admin"):
            messagebox.showwarning("Access Denied", "Please login as Admin first.")
            self.controller.navigate("AdminLoginPage", add_to_history=False)
            return

        # Refresh dashboard each time
        self.set_active_button("🏠   Dashboard")
        self.show_dashboard_page()
    ...
    def create_sidebar(self):
        ...
        for text, command in buttons:
            btn = tk.Button(
                ...
                command=lambda c=command, t=text: self.sidebar_click(c, t)
            )
            btn.pack(fill="x")
    ...
    def set_active_button(self, btn_text):
        """Highlight active menu item without calling the command."""
    ...
    def sidebar_click(self, command, btn_text):
        self.set_active_button(btn_text)
        command()
    ...
    def show_dashboard_page(self):
        ...
        self.chart_fig, ax = plt.subplots(figsize=(6.5, 3.2))
        ...
        canvas = FigureCanvasTkAgg(self.chart_fig, chart_frame)
    ...
    def logout(self):
        if messagebox.askyesno("Logout", "Are you sure you want to logout?"):
            self.controller.session["admin"] = None
